...
The IBM Cloud account is added to Cloudability by the deployable architecture, but verification is failing. Verification may take a little longer or it may need to be skipped when running the deployable architecture.
{: shortdesc}
...
Alternatively, account verification is not an essential step for the deployment and can be disabled in the deployable architecture configuration so that the deployable architecture runs cleanly.  To disable verification visit the **Optional** tab of the deployable architecture configuration settings and set `skip_verification` to **true** and then re-run the deployment.
